...
d3.csv(csvPath).then(data => {
    ...
    // For each series draw its own line in brush
    valueColumns.forEach(col => {
        const line = d3
            .line()
            .x(d => xTime(d[dateColumn]))
    ...
    setTimeout(() => {
        const help = document.getElementById("brushHelp");
...
function updateBars(startDate, endDate) {
    ...
    const fmt = d3.timeFormat("%Y-%m-%d %H:%M");
    if(windowLabel) {
        windowLabel.textContent = `Window: ${fmt(startDate)}  →  ${fmt(endDate)}`;
    }
    ...
    const bars = barG.selectAll("rect.bar").data(seriesData, d => d.key);
    ...
    bars.enter()
        ...
        .merge(bars)
        .transition()
        .duration(50) // faster transition for smooth play
        .attr("x", d => xBand(d.key))
        .attr("width", xBand.bandwidth())
        .attr("y", d => (d.pct >= 0 ? yBar(d.pct) : zeroY))
        .attr("height", d => Math.abs(yBar(d.pct) - zeroY))
        .attr("fill", d => color(d.key));

    bars.exit().remove();

    const labels = barG.selectAll("text.bar-label").data(seriesData, d => d.key);
